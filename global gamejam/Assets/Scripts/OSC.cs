...
public class UDPPacketIO 
{
    ...
    private OSCObjectMode oscMode = OSCObjectMode.Full;
	...
	public UDPPacketIO(string hostIP, int remotePort, int localPort, OSCObjectMode inOscMode = OSCObjectMode.Full){
	...
	public bool Open()
	{
		try
		{
            ...
            if (oscMode != OSCObjectMode.SenderOnly)
            {
	...
	{
        if (oscMode != OSCObjectMode.ReceiverOnly)
        {
            if (!IsOpen())
                Open();
            if (!IsOpen())
...
public class OSC : MonoBehaviour
{
    ...
    public OSCObjectMode AutoInitializeOSCMode = OSCObjectMode.Full;
    [Space (20)]
    ...
    void Awake()
    {
        ...
        if (AutoInitialize)
        {
            InitializeMe(outIP, inPort, outPort,AutoInitializeOSCMode);
        }
    }

    public void InitializeMe(string inTargetIP, int inInPort, int inOutPort, OSCObjectMode inOSCObjectMode = OSCObjectMode.Full)
    {
        ...
        OscPacketIO = new UDPPacketIO(outIP, outPort, inPort, inOSCObjectMode);
